...
=====================================================================
Workflow environment based on Raspbian $(cat /etc/os-release)
EOF

# get docker registry certificate
mkdir -p /etc/docker/ /etc/docker/certs.d/ "/etc/docker/certs.d/$docker_registry"
wget -O "/etc/docker/certs.d/$docker_registry/ca.crt" "$packet_base_url/ca.pem"
...
docker run -dit --net host \
	"$docker_registry/fluent-bit:1.3-arm" \
...
docker run --privileged  \
	...
	--net host \
	"$docker_registry/tink-worker:armv7"
